...
if (isset($_POST['simpan'])) {
    ...
    $tanggal = date("Y-m-d H:i:s");// Otomatis mengambil waktu server saat ini
    $username = $_SESSION['username'];// Otomatis mengambil nama admin yang sedang login
    $gambar = '';
    $nama_gambar = $_FILES['gambar']['name'];// Mengambil nama file dari form
    ...
    if ($nama_gambar != '') {
        $cek_upload = upload_foto($_FILES["gambar"]);// Proses upload ke folder img/

        if ($cek_upload['status']) {
            $gambar = $cek_upload['message'];// Nama file inilah yang disimpan ke database
        } else {
            echo "<script>
